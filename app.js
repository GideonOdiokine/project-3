...
let users = [
	...
	users.forEach((user)=>{
		let carousel_content = `
			<div class="carousel-users-content">
	  			<img class="user-image" src="`+ user.avatar_url +`">
	  			<h3 class="user-name">`+ user.login +`</h3>
	  		</div>`;
		let carousel_container = document.querySelector('.carousel-users');

		carousel_container.insertAdjacentHTML('beforeend', carousel_content);
	
	})

	let carousel_list = document.querySelectorAll('.carousel-users .carousel-users-content');
	let index = 0;
	window.show = function(increase) {
	index = index + increase;
	index = Math.min(Math.max(index,0), carousel_list.length-1);
	carousel_list[index].scrollIntoView({behavior: 'smooth'});

	// For automatic slide
	let carousel = document.querySelectorAll('.carousel-users .carousel-users-content');

	let nextImageDelay= 5000;
	let currentImageCounter=0;

	// carousel[currentImageCounter].style.display="block";
	setInterval(nextImage, nextImageDelay);

	function nextImage(){
	carousel[currentImageCounter].style.display="none";
	currentImageCounter=(currentImageCounter + 1) % carousel.length;
	carousel[currentImageCounter].style.display="block";
	}
}
}())